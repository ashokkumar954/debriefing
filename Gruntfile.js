...
module.exports = function (grunt) {
    ...
    pkgObject.version = getReleaseVersion();
...
// OJET build (release)
    grunt.registerTask('ojetBuild', 'Run ojet build (release)', function () {
        const done = this.async();
        const ojet = child_process.spawn('./node_modules/.bin/ojet', ['build', '--release'], {stdio: 'inherit'});
...
// OJET build (dev) – equivalent to oraclejet-build:dev
    grunt.registerTask('ojetBuildDev', 'Run ojet build (dev)', function () {
        const done = this.async();
        const ojet = child_process.spawn('./node_modules/.bin/ojet', ['build'], {stdio: 'inherit'});
        ojet.on('close', code => (code === 0 ? done() : done(false)));
    });
...
// Match the name your script calls
    ...
    function getReleaseVersion(name) {
        //Check FABS release version in environment variables
        const versionEnvironmentVariable = 'CI_RELEASE_VERSION';

        if (process.env.hasOwnProperty(versionEnvironmentVariable) && process.env[versionEnvironmentVariable].length > 0) {
            const fabsReleaseVersion = process.env[versionEnvironmentVariable];
            const modifiedFabsReleaseVersion = processVersion(fabsReleaseVersion);

            return modifiedFabsReleaseVersion;
        }

        //Otherwise use local build number
        return pkgObject.version + '-' + new Date().getTime();
    }
